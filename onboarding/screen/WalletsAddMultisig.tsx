...
type Cosigner = [string, string, string, string?];
...
const WalletsAddMultisigStep2 = () => {
  ...
  const [cosigners, setCosigners] = useState<Cosigner[]>([]); // array of cosigners user provided. if format [cosigner, fp, path]
  ...
  const viewKey = (cosigner: Cosigner) => {
  ...
  const getXpubCacheForMnemonics = (seed: string, passphrase?: string) => {
    const path = getPath();
    return staticCache[seed + path + passphrase] || setXpubCacheForMnemonics(seed, passphrase);
  };

  const setXpubCacheForMnemonics = (seed: string, passphrase?: string) => {
    const path = getPath();
    const w = new MultisigHDWallet();
    ...
    return (
      <View>
        <MultipleStepsListItem
          ...
          leftText={el.index === 0 ? 'PB Key' : loc.formatString(loc.multisig.vault_key, { number: el.index + 1 })}
          ...
          rightButton={
            el.index === 0
              ? null
              : {
                  disabled: el.index === 0 || vaultKeyData.isLoading,
